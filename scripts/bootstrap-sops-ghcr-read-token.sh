...
secret_path="apps/homelab-api/envs/${env_name}/ghcr-read-token-secret.enc.yaml"
generator_path="apps/homelab-api/envs/${env_name}/ghcr-read-token-secret-generator.yaml"
...
if [[ "$mode" == "--from-cluster" ]]; then
  ...
  token_b64="$(kubectl -n homelab-api get secret homelab-api-ghcr-read -o jsonpath='{.data.GHCR_READ_TOKEN}' 2>/dev/null || true)"
  if [[ -z "$token_b64" ]]; then
    echo "cluster secret homelab-api-ghcr-read is missing GHCR_READ_TOKEN"
    exit 1
  fi
  token="$(printf '%s' "$token_b64" | base64 -d)"
else
  read -rsp "GHCR_READ_TOKEN: " token
  echo
fi

if [[ -z "$token" ]]; then
  echo "GHCR_READ_TOKEN cannot be empty"
  exit 1
fi
...
kind: ksops
metadata:
  name: ghcr-read-token-secret-generator
  annotations:
    config.kubernetes.io/function: |
      exec:
        path: ksops
files:
  - ghcr-read-token-secret.enc.yaml
EOF

if ! grep -qE '^[[:space:]]*-[[:space:]]*ghcr-read-token-secret-generator\.yaml[[:space:]]*$' "$kustomization_path"; then
  awk '
    /^generators:/ && inserted == 0 { print; print "  - ghcr-read-token-secret-generator.yaml"; inserted = 1; next }
    /^commonLabels:/ && inserted == 0 { print "generators:"; print "  - ghcr-read-token-secret-generator.yaml"; inserted = 1 }
    { print }
